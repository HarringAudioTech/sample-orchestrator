...
class TestProject(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_project_model_instance = ProjectModel(
    ...
    def test_project_init_not_found(self, mock_get_db_notfound):
        ...
        with self.assertRaisesRegex(
            ValueError, f"Project with id {self.project_id} not found"
        ):
    ...
    def test_add_recording_successful_no_session_provided(
        ...
        with patch(
            "src.core.project.RecordingModel", side_effect=recording_model_factory
        ) as PatchedRecordingModel:
    ...
    def test_add_recording_file_not_found(self, mock_get_db_fnf, mock_os_exists_fnf):
        ...
        with self.assertRaisesRegex(
            FileNotFoundError, f"Recording file not found: {file_path}"
        ):
    ...
    @patch("src.core.project.os.path.exists", return_value=True)
    @patch(
        "src.core.project.librosa.load",
        side_effect=librosa.LibrosaError("Librosa load failed"),
    )
    # Patched RecordingModel will be handled by the side_effect setup
    @patch("src.core.project.get_db")
    def test_add_recording_librosa_error(
        self, mock_get_db_le, mock_librosa_load_err, mock_os_exists_le
    ):
        def recording_model_factory(**kwargs):
            instance = MagicMock(name="RecordingModelInstance_Mock")
            kwargs.setdefault("id", None)
        ...
        with patch(
            ...
            new_recording = project.add_recording(
                file_path="/test/audio.wav", name="Librosa Error Case"
            )
    ...
    # Patched RecordingModel will be handled by the side_effect setup
    @patch("src.core.project.get_db")
    def test_add_recording_sqlalchemy_error(
        self,
        mock_get_db_sqlae,
        mock_get_duration_sqlae,
        mock_librosa_load_sqlae,
        mock_os_exists_sqlae,
    ):
        def recording_model_factory(**kwargs):
            instance = MagicMock(name="RecordingModelInstance_Mock")
            kwargs.setdefault("id", None)
        ...
        with patch(
            ...
            self.mock_db_session.commit.side_effect = SQLAlchemyError(
    ...
    def test_get_recording_found_no_session_provided(self, mock_get_db_gr):
        ...
        mock_filter_rec = mock_query_rec.filter(
            ANY
        )  # SUT uses .filter(R.id == rid, R.project_id == pid)
    ...
    def test_get_recording_found_session_provided(self):
        ...
        mock_recording = RecordingModel(
            id=11, name="Found Rec Session", project_id=self.project_id
        )
    ...
    def test_list_recordings_successful(self, mock_get_db_lr):
        ...
        mock_filter_rec = mock_query_rec.filter(
            ANY
        )  # SUT uses .filter(R.project_id == self.project_id)
    ...
    def test_create_midi_capture_session_successful(
    ...
    ):  # Removed MockMidiRecorder_class_patch
        ...
        MidiRecorder.return_value = (
            mock_recorder_instance  # When MidiRecorder(...) is called, return this
        )
    ...
    def test_list_midi_capture_sessions_successful(self, mock_get_db_lmcs):
        ...
        mock_sessions_list = [
            MidiCaptureSessionModel(id=1, name="Session 1", project_id=self.project_id)
        ]
    ...
    def test_get_midi_capture_session_found(self, mock_get_db_gmcs):
        ...
        session_id = 5
        mock_session = MidiCaptureSessionModel(
            id=session_id, name="Target Session", project_id=self.project_id
        )

        mock_query_sess = self.mock_db_session.query(MidiCaptureSessionModel)
        mock_filter_sess = mock_query_sess.filter(
            ANY
        )  # SUT uses .filter(MCSM.id == sid, MCSM.project_id == pid)
    ...
    def test_get_midi_files_for_session_successful(self, mock_get_db_gmfs):
        ...
        session_id = 7
        mock_capture_session = MidiCaptureSessionModel(
            id=session_id, name="Session With Files", project_id=self.project_id
        )
        ...
        mock_midi_files_list = [
            MidiFileModel(
                id=1,
                midi_capture_session_id=session_id,
                midi_device_id=1,
                channel_number=0,
                midi_data=b"data",
            )
    ...
    @patch("src.core.project.get_db")
    def test_get_midi_files_for_session_not_found_or_not_project_session(
        self, mock_get_db_gmfs_nf
    ):
        ...
        self.mock_db_session.query(
            MidiFileModel
        ).filter().order_by().all.assert_not_called()  # Check general filter call
