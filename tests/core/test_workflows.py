...
class MockWorkflowAlpha(BaseWorkflow):
    ...
    @property
    def stages_definition(self) -> List[Dict[str, Any]]: return self._stages_definition

class MockWorkflowBeta(BaseWorkflow): # Different class, same derived registry key if not careful
...
def test_register_workflow_reregistration(caplog):
    ...
    class MockWorkflowAlphaVariant(BaseWorkflow): # Same class name structure as MockWorkflowAlpha
        @property
        def name(self) -> str: return "Variant Alpha"
        @property
        def description(self) -> str: return "A variant."
        @property
        def stages_definition(self) -> List[Dict[str, Any]]: return []
        ...
        # The warning is more about "if a key is already there, it's overwritten".
    ...
    # by directly manipulating the registry for setup, then calling register_workflow.
    ...
    with patch('src.core.workflows._camel_to_snake', return_value=expected_key_alpha):
        with caplog.at_level(logging.WARNING):
            register_workflow(MockWorkflowBeta) # This will use expected_key_alpha due to patch

    assert expected_key_alpha in WORKFLOW_REGISTRY
    assert WORKFLOW_REGISTRY[expected_key_alpha] == MockWorkflowBeta # Overwritten
    
    assert any(
        f"Workflow registry key '{expected_key_alpha}' (derived from class '{MockWorkflowBeta.__name__}') is already registered. Overwriting" in record.message
...
def test_base_workflow_run_method(mock_execute_stage_chain):
    ...
    assert WorkflowClass is not None, f"{workflow_key} not found after attempting registration."
    
    workflow_instance = WorkflowClass()
    
    initial_data_val = "/path/to/some/file.wav"
    initial_data_type_val = DATA_TYPE_FILE_PATH # ExampleSlicingWorkflow's first stage (noise_reduction) expects audio_buffer_mono
                                              ...
                                              # For testing `run` itself, we care that it *calls* execute_stage_chain correctly.
    ...
    # The second stage 'slicing' expects 'file_path'. This workflow is flawed as defined.
    ...
    # For this test, we only care that execute_stage_chain is called with the right things from workflow.run().
    ...
    result = mock_workflow_alpha_instance.run(
        ...
        initial_data_type=DATA_TYPE_FILE_PATH, # Assume this matches first stage of MockWorkflowAlpha
