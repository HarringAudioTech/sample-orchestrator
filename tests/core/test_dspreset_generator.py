...
class TestDecentSamplerPresetGenerator(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.patcher_et_write = patch("xml.etree.ElementTree.ElementTree.write") # Path to ET.write within the module it's used
        self.mock_et_write = self.patcher_et_write.start()


    def tearDown(self) -> None:
        """Clean up test fixtures after each test method."""
        if os.path.exists(self.test_output_base_dir): # This part is for the integration-style tests
            shutil.rmtree(self.test_output_base_dir)
    ...
    def test_sanitize_filename(self) -> None:
        ...
        # Input with spaces
        self.assertEqual(generator._sanitize_filename("file with spaces.wav"), "file_with_spaces.wav")
        # Input with mixed case
        self.assertEqual(generator._sanitize_filename("MixedCaseFile.WAV"), "mixedcasefile.wav")
        # Input that is already sanitized
        self.assertEqual(generator._sanitize_filename("already_sanitized.wav"), "already_sanitized.wav")
        # Input with special characters
        self.assertEqual(generator._sanitize_filename("file-with!@#$%^&*().wav"), "file-with.wav")
        self.assertEqual(generator._sanitize_filename("another_one_with_üöä.mp3"), "another_one_with_.mp3")
        ...
        self.assertEqual(generator._sanitize_filename("-leading-trailing-.wav"), "leading-trailing-.wav")

    # --- New tests using mocks for generate_preset ---

    def _setup_mock_project_data(self, num_recordings=1, num_samples_per_recording=1, with_mappings=True, with_bg_image=True):
    ...
    def test_generate_preset_basic_success(self):
        self._setup_mock_project_data(num_recordings=1, num_samples_per_recording=1, with_mappings=True, with_bg_image=True)

        # For these tests, the output_base_dir is just a string, os.makedirs is mocked
        output_base_dir = "/mocked/output"
        generator = DecentSamplerPresetGenerator(self.mock_project, self.instrument_data, output_base_dir)
        ...
        self.assertIsNotNone(self.instrument_data.ui_background_image_path) # Ensure path exists for type checker
        ...
        self.mock_shutil_copy2.assert_any_call(
            ...
            os.path.join(expected_samples_dir, generator._sanitize_filename(os.path.basename(mock_sample.file_path)))
        )

        # Assert ET.ElementTree.write call
        expected_xml_path = os.path.join(expected_instrument_dir, f"{sanitized_instrument_name}.dspreset")
        ...
        self.assertIsNotNone(root_element.find(f"ui/tab/background[@image='Artwork{os.sep}background.png']"))
        ...
        expected_sample_xml_path = os.path.join("Samples", generator._sanitize_filename(os.path.basename(mock_sample.file_path)))
        self.assertIsNotNone(root_element.find(f"groups/group/sample[@path='{expected_sample_xml_path}']"))
    ...
    def test_generate_preset_no_ui_background_image(self):
        ...
        artwork_copy_calls = [c for c in self.mock_shutil_copy2.call_args_list if "Artwork" in c[0][1]]
        self.assertEqual(len(artwork_copy_calls), 0, "Should not attempt to copy artwork if no path is set")
    ...
    def test_generate_preset_missing_ui_background_image_file(self, mock_print):
        ...
        mock_print.assert_any_call(f"Warning: UI background image not found at {self.instrument_data.ui_background_image_path}, skipping.")
    ...
    def test_generate_preset_missing_sample_file(self, mock_print):
        self._setup_mock_project_data(num_samples_per_recording=2) # Ensure there's another sample
        ...
        mock_print.assert_any_call(f"Warning: Sample file not found at {mock_sample_to_miss.file_path}, skipping.")
        ...
        self.assertNotEqual(samples_in_xml[0].get("path"), os.path.join("Samples", generator._sanitize_filename(os.path.basename(mock_sample_to_miss.file_path))))
    ...
    def test_generate_preset_ioerror_on_artwork_copy(self, mock_print):
        ...
        self.mock_shutil_copy2.side_effect = lambda src, dst: (_ for _ in ()).throw(IOError("Test IOError on artwork")) if "Artwork" in dst else None
        ...
        mock_print.assert_any_call(f"Error copying artwork from {self.instrument_data.ui_background_image_path}: Test IOError on artwork")
    ...
    def test_generate_preset_ioerror_on_sample_copy(self, mock_print):
        ...
        mock_print.assert_any_call(f"Error copying sample file {mock_sample_to_fail_copy.file_path}: Test IOError on sample copy")
        ...
        self.assertNotEqual(samples_in_xml[0].get("path"), os.path.join("Samples", generator._sanitize_filename(os.path.basename(mock_sample_to_fail_copy.file_path))))
    ...
    def test_generate_preset_ioerror_on_xml_write(self, mock_print):
        ...
        sanitized_instrument_name = "mocked_instrument"
        expected_xml_path = os.path.join(output_base_dir, sanitized_instrument_name, f"{sanitized_instrument_name}.dspreset")
        mock_print.assert_any_call(f"Error writing XML file to {expected_xml_path}: Test IOError on XML write")
