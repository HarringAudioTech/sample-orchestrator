...
from src.core.audio_processor import detect_and_slice_recording, get_audio_details
...
def test_project_and_recording(db_session: Session):
    ...
    # Get actual audio details from the dummy file to make the test more realistic
    try:
...
# Patching 'src.core.audio_processor.source' and 'src.core.audio_processor.notes'

@patch('src.core.audio_processor.notes')
@patch('src.core.audio_processor.source')
@patch('src.core.audio_processor.wave.open') # Also mock wave.open for the slicing part
def test_detect_and_slice_success(mock_wave_open_slicing, mock_aubio_source, mock_aubio_notes_obj, test_project_and_recording, db_session: Session, temp_output_dir_for_slicing, capsys):
    ...
    mock_s_instance.duration = int((recording.duration_seconds if recording.duration_seconds else 0.023) * mock_s_instance.samplerate) # total frames
    ...
    # Total frames read before this chunk = 256. So absolute start_frame = 256 + notes_o.get_last_pos()
    ...
    # The code calculates start_frame = total_frames_read - read + int(notes_o.get_last_pos())
    simulated_notes_output = [
        [], # No notes in 1st read
        [(60, 100, detected_note_frame_in_chunk)], # One note in 2nd read (MIDI 60, vel 100)
    ...
    # This mock will be used when wave.open is called to slice and save the sample.
    mock_wf_slice_read = MagicMock()
    mock_wf_slice_read.__enter__.return_value.setpos = MagicMock()
    mock_wf_slice_read.__enter__.return_value.readframes.return_value = b'\x00\x00' * 512 # Dummy audio data for slice
    ...
    # The first call (read) should use mock_wf_slice_read, second (write) mock_wf_slice_write.
    mock_wave_open_slicing.side_effect = [mock_wf_slice_read, mock_wf_slice_write]
    ...
    samples = db_session.query(SampleModel).filter(SampleModel.recording_id == recording.id).all()
    assert len(samples) == 1
    sample = samples[0]
    assert sample.name.startswith(f"rec_{recording.id}_sample_midi60")
    assert sample.midi_pitch == 60
    assert sample.file_path.startswith(os.path.join(output_dir, f"rec_{recording.id}_sample_midi60"))
    ...
    # The logic is: end_frame = min(detected_notes[i]["start_frame"] + samplerate, total_frames_read)
    expected_start_frame = 256 + detected_note_frame_in_chunk # 356
    expected_end_frame = min(expected_start_frame + mock_s_instance.samplerate, mock_s_instance.duration)

    assert sample.start_time_seconds == float(expected_start_frame) / mock_s_instance.samplerate
    assert sample.end_time_seconds == float(expected_end_frame) / mock_s_instance.samplerate
    ...
    # 4. Aubio calls
    mock_aubio_source.assert_called_with(DUMMY_AUDIO_PATH, mock_s_instance.samplerate, ANY) # hop_size=256
    mock_aubio_notes_obj.assert_called_with("default", ANY, ANY, mock_s_instance.samplerate) # win_size, hop_size

    # 5. Wave slicing calls
    # First call to wave.open (read mode)
    mock_wave_open_slicing.assert_any_call(DUMMY_AUDIO_PATH, 'rb')
    mock_wf_slice_read.__enter__.return_value.setpos.assert_called_once_with(expected_start_frame)
    mock_wf_slice_read.__enter__.return_value.readframes.assert_called_once_with(expected_end_frame - expected_start_frame)
    
    # Second call to wave.open (write mode)
    mock_wave_open_slicing.assert_any_call(sample.file_path, 'wb')
    mock_wf_slice_write.__enter__.return_value.setnchannels.assert_called_once_with(ANY) # Or specific like recording.channels
    mock_wf_slice_write.__enter__.return_value.setsampwidth.assert_called_once()
    mock_wf_slice_write.__enter__.return_value.setframerate.assert_called_once_with(mock_s_instance.samplerate)
    mock_wf_slice_write.__enter__.return_value.writeframes.assert_called_once()
    
    # Check console output for success message
    captured_output = capsys.readouterr().out
    assert f"Successfully processed recording {recording.id}" in captured_output


def test_detect_and_slice_no_notes_found(test_project_and_recording, db_session: Session, temp_output_dir_for_slicing, capsys):
    project, recording = test_project_and_recording
    output_dir = temp_output_dir_for_slicing

    with patch('src.core.audio_processor.source') as mock_aubio_source, \
         patch('src.core.audio_processor.notes') as mock_aubio_notes_obj:

        mock_s_instance = MagicMock()
        mock_s_instance.samplerate = 44100
        mock_s_instance.duration = 1024
        mock_s_instance.side_effect = [(MagicMock(), 256)] * 4 + [(MagicMock(), 0)] # Simulate reads
        mock_aubio_source.return_value = mock_s_instance
...
def test_detect_and_slice_file_not_found(test_project_and_recording, db_session: Session, temp_output_dir_for_slicing, capsys):
    project, recording = test_project_and_recording
    ...
    captured_output = capsys.readouterr().out
    assert f"File path for recording {recording.id} is invalid or file does not exist" in captured_output


@patch('src.core.audio_processor.get_audio_details', return_value=(0,0)) # Force samplerate to be 0
def test_detect_and_slice_invalid_samplerate(mock_get_details, test_project_and_recording, db_session: Session, temp_output_dir_for_slicing, capsys):
    project, recording = test_project_and_recording
    output_dir = temp_output_dir_for_slicing
    ...
    captured_output = capsys.readouterr().out
    assert f"Could not determine samplerate for {recording.file_path}" in captured_output


@patch('src.core.audio_processor.source')
@patch('src.core.audio_processor.notes')
@patch('src.core.audio_processor.wave.open') # Mock wave for slicing
def test_detect_and_slice_slicing_error(mock_wave_open_slicing, mock_aubio_source, mock_aubio_notes_obj, test_project_and_recording, db_session: Session, temp_output_dir_for_slicing, capsys):
    project, recording = test_project_and_recording
    output_dir = temp_output_dir_for_slicing
    ...
    mock_s_instance.samplerate = 44100
    mock_s_instance.duration = 1024
    mock_s_instance.side_effect = [(MagicMock(), 256)] * 2 + [(MagicMock(), 0)] * 3 # Simulate reads
    mock_aubio_source.return_value = mock_s_instance

    mock_notes_o_instance = MagicMock()
    mock_notes_o_instance.get_last_pos.return_value = 100
    mock_notes_o_instance.side_effect = [[], [(60, 100, 100)], [], []] # One note
    mock_aubio_notes_obj.return_value = mock_notes_o_instance
    ...
    assert f"Successfully processed recording {recording.id}" in captured_output # Outer function completes
    # This highlights a potential refinement: if all samples fail to slice, status could be "failed".

# Test get_audio_details helper (not part of detect_and_slice directly but used by it)
@patch('src.core.audio_processor.wave.open')
@patch('src.core.audio_processor.source') # Mock aubio source for get_audio_details
def test_get_audio_details_aubio_success(mock_aubio_source_gad, mock_wave_open_gad):
    mock_s_gad = MagicMock()
    mock_s_gad.samplerate = 48000
    mock_s_gad.duration = 96000 # total frames
    mock_aubio_source_gad.return_value = mock_s_gad

    samplerate, total_frames = get_audio_details("fake_path.wav")
    assert samplerate == 48000
    assert total_frames == 96000
    mock_wave_open_gad.assert_not_called()


@patch('src.core.audio_processor.source', side_effect=Exception("Aubio error"))
@patch('src.core.audio_processor.wave.open')
def test_get_audio_details_aubio_fails_wave_success(mock_wave_open_gad, mock_aubio_source_gad_fails):
...
@patch('src.core.audio_processor.source', side_effect=Exception("Aubio error"))
@patch('src.core.audio_processor.wave.open', side_effect=Exception("Wave error"))
def test_get_audio_details_all_fail(mock_wave_open_gad_fails, mock_aubio_source_gad_fails, capsys):
    samplerate, total_frames = get_audio_details("fake_path.wav")
    assert samplerate == 0
    assert total_frames == 0
    captured = capsys.readouterr().out
    assert "Error getting audio details for fake_path.wav with aubio: Aubio error" in captured
    assert "Error getting audio details for fake_path.wav with wave: Wave error" in captured
