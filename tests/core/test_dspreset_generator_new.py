...
class TestDecentSamplerPresetGeneratorNew(unittest.TestCase):
    ...
    def test_sanitize_filename(self):
        ...
        self.assertEqual(self.generator._sanitize_filename("My Instrument Name"), "my_instrument_name")
        self.assertEqual(self.generator._sanitize_filename("MY_UPPER_CASE_NAME"), "my_upper_case_name")
        self.assertEqual(self.generator._sanitize_filename("already_sanitized"), "already_sanitized")
        # Current implementation of _sanitize_filename is very simple:
        # name.replace(" ", "_").lower()
        # It does not remove special characters other than spaces.
        self.assertEqual(self.generator._sanitize_filename("name with-hyphens_and_underscores"), "name_with-hyphens_and_underscores")
        self.assertEqual(self.generator._sanitize_filename("Name With Numbers 123"), "name_with_numbers_123")
        self.assertEqual(self.generator._sanitize_filename(" Special Chars!@#$ "), "_special_chars!@#$_") # Corrected: added trailing underscore
    ...
    def test_generate_preset_successful_basic_structure(self, mock_exists, mock_xml_write, mock_copy, mock_makedirs):
        ...
        sanitized_instrument_name = self.generator._sanitize_filename(self.instrument_data.name)
        ...
        mock_copy.assert_any_call(self.instrument_data.ui_background_image_path, expected_dest_artwork_path)
        ...
        mock_copy.assert_any_call(self.mock_sample_model_1.file_path, expected_dest_sample_path)

        # Assertion for XML file writing
        expected_dspreset_filename = sanitized_instrument_name + ".dspreset"
        expected_dspreset_path = os.path.join(expected_instrument_dir, expected_dspreset_filename)
        ...
        mock_xml_write.assert_called_once_with(expected_dspreset_path, encoding="UTF-8", xml_declaration=True)
    ...
    @patch('shutil.copy2')
    @patch('os.path.exists', return_value=True)
    def test_generate_preset_xml_content_detailed(self, mock_exists, mock_copy, mock_makedirs, mock_ET_ElementTree_class):
        ...
        self.assertTrue(mock_ET_ElementTree_class.called, "ET.ElementTree constructor was not called.")
        ...
        expected_artwork_path = os.path.join("Artwork", os.path.basename(self.instrument_data.ui_background_image_path))
        ...
        expected_sample1_path = os.path.join("Samples", os.path.basename(self.mock_sample_model_1.file_path))
        ...
        self.assertEqual(sample1_xml.get("loKey"), str(self.mock_sample_mapping_item_1.key_range_start))
        self.assertEqual(sample1_xml.get("hiKey"), str(self.mock_sample_mapping_item_1.key_range_end))
        ...
        expected_sample2_path = os.path.join("Samples", os.path.basename(mock_sample_model_2.file_path))
        ...
        self.assertEqual(sample2_xml.get("loKey"), str(mock_sample_mapping_item_2.key_range_start))
        self.assertEqual(sample2_xml.get("hiKey"), str(mock_sample_mapping_item_2.key_range_end))
        ...
        expected_dspreset_filename = sanitized_instrument_name + ".dspreset"
        expected_dspreset_path = os.path.join(expected_instrument_dir, expected_dspreset_filename)
        mock_tree_instance.write.assert_called_once_with(expected_dspreset_path, encoding="UTF-8", xml_declaration=True)
    ...
    @patch('shutil.copy2')
    @patch('os.path.exists', return_value=True)
    def test_generate_preset_no_ui_background_image(self, mock_exists, mock_copy, mock_makedirs, mock_ET_ElementTree_class):
        ...
        for call_item in mock_copy.call_args_list:
            ...
            if "Artwork" in args[1]:
                ...
                break
        self.assertFalse(artwork_copy_called, "shutil.copy2 should not be called for artwork if no path is provided.")
        ...
        self.assertEqual(len(all_background_images), 0, "No background image attribute should be present.")
        if tab_element: # Tab might still exist
                self.assertIsNone(tab_element.find("background"), "Background element itself should be absent or have no image attr.")
    ...
    def test_generate_preset_no_recordings_or_samples(self, mock_exists, mock_copy, mock_makedirs, mock_ET_ElementTree_class):
        self.mock_project.project_model.recordings = []
        ...
        groups_element = written_tree_root.find("groups")
        self.assertIsNotNone(groups_element)
        self.assertEqual(len(groups_element.findall("group")), 0, "Groups element should be empty if no recordings.")
        ...
        self.assertIn("No recordings found", printed_output, "Expected info message about no recordings not found in print output.")
    ...
    def test_generate_preset_missing_ui_background_image_file(self, mock_print, mock_exists, MockElementTree, mock_copy, mock_makedirs):
        ...
        self.assertTrue(any(f"WARNING: Artwork source file not found: {self.instrument_data.ui_background_image_path}" in str(call_args) for call_args in mock_print.call_args_list))
        ...
        all_background_images = written_tree_root.findall(".//ui/tab/background[@image]")
        self.assertEqual(len(all_background_images), 0, "No background image should be in XML if source file is missing.")
        ...
        for call_item in mock_copy.call_args_list:
            ...
            if args_copy[0] == self.instrument_data.ui_background_image_path:
                artwork_copy_called = True
                break
        self.assertFalse(artwork_copy_called, "shutil.copy2 should not be called for missing artwork.")
    ...
    def test_generate_preset_missing_sample_file(self, mock_print, mock_exists, MockElementTree, mock_copy, mock_makedirs):
        ...
        self.assertTrue(any(f"WARNING: Sample source file not found, skipping: {self.mock_sample_model_1.file_path}" in str(call_args) for call_args in mock_print.call_args_list))
        ...
        self.assertEqual(len(sample_elements), 0, "No sample elements should be in XML if the source file is missing.")
        ...
        for call_item in mock_copy.call_args_list:
            ...
            if args_copy[0] == self.mock_sample_model_1.file_path:
                ...
                break
        self.assertFalse(sample_copy_called, "shutil.copy2 should not be called for a missing sample file.")
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.print')
    def test_generate_preset_oserror_on_makedirs(self, mock_print, mock_exists, MockElementTree, mock_copy, mock_makedirs_oserror):
        self.generator.generate_preset()
        # Verify error message is printed
        self.assertTrue(any("ERROR: Could not create instrument directories: Test OSError" in str(call_args) for call_args in mock_print.call_args_list))
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.print')
    def test_generate_preset_ioerror_on_artwork_copy(self, mock_print, mock_exists, MockElementTree, mock_copy_ioerror, mock_makedirs):
        self.instrument_data.ui_background_image_path = "/path/to/artwork.jpg"
        ...
        artwork_filename = os.path.basename(self.instrument_data.ui_background_image_path)
        self.assertTrue(any(f"ERROR: Could not copy artwork file {artwork_filename}: Test IOError on artwork copy" in str(call_args) for call_args in mock_print.call_args_list))
        ...
        was_sample_copied = any(call_item.args[0] == sample_path_to_check for call_item in mock_copy_ioerror.call_args_list if call_item.args[0] != self.instrument_data.ui_background_image_path) # Exclude the failing artwork call
        self.assertTrue(was_sample_copied, "Sample should still be attempted to be copied if artwork copy fails")
    ...
    @patch('builtins.print')
    def test_generate_preset_ioerror_on_sample_copy(self, mock_print, mock_exists, MockElementTree, mock_copy, mock_makedirs):
        ...
        mock_failing_sample.sample_mapping_items = [self.mock_sample_mapping_item_1] # Reuse mapping

        successful_sample_path = self.mock_sample_model_1.file_path # from setUp
        self.mock_sample_model_1.sample_mapping_items = [self.mock_sample_mapping_item_1] # Reuse
        ...
        self.assertTrue(any(f"ERROR: Could not copy sample file {failing_sample_filename}: Test IOError on sample copy" in str(call_args) for call_args in mock_print.call_args_list))
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.print')
    def test_generate_preset_ioerror_on_xml_write(self, mock_print, mock_exists, MockElementTree, mock_copy, mock_makedirs):
        ...
        self.assertTrue(any("ERROR: Could not write .dspreset file: Test IOError on XML write" in str(call_args) for call_args in mock_print.call_args_list))
