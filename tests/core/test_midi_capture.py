...
from src.core.midi_capture import sanitize_filename, list_available_midi_devices, get_midi_device_by_name, MidiRecorder
...
class TestMidiCaptureUtilities(unittest.TestCase):
    ...
    def test_sanitize_filename(self):
        ...
        self.assertEqual(sanitize_filename("Device-With-Hyphens!@#.mid"), "Device-With-Hyphens.mid")
        self.assertEqual(sanitize_filename("  Leading Trailing Spaces  "), "__Leading_Trailing_Spaces__") # Corrected assertion
    ...
    def test_list_available_midi_devices_new_and_existing(self, mock_datetime_module, mock_get_input_names):
        mock_db_session = MagicMock()

        mock_get_input_names.return_value = ["MIDI Device 1", "MIDI Device 2", "New Device 3"]

        mock_existing_device_1 = MidiDevice(id=1, name="MIDI Device 1", system_identifier="sys_id_1")
        mock_existing_device_2 = MidiDevice(id=2, name="MIDI Device 2", system_identifier="sys_id_2")
    ...
    def test_list_available_midi_devices_sqlalchemy_exception_on_commit(self, mock_get_input_names_sql):
        mock_db_session = MagicMock()
    ...
    def test_get_midi_device_by_name_sqlalchemy_error(self):
        mock_db_session = MagicMock()
        mock_db_session.query(MidiDevice).filter().first.side_effect = SQLAlchemyError("Query error")
...
class TestMidiRecorder(unittest.TestCase):
    def setUp(self):
        ...
        mock_first_result.first.return_value = self.mock_project_instance # first() returns the project
        ...
        self.mock_found_device = MidiDevice(id=1, name="MIDI Device 1", system_identifier="sys_id_1_mr")
    ...
    @patch('src.core.midi_capture.datetime', wraps=datetime)
    def test_recorder_initialization_device_found(self, mock_datetime_module, mock_mido_open_input, mock_get_device_by_name):
        ...
        recorder = MidiRecorder(self.project_id, self.selected_device_names, self.session_name, self.mock_db_session)
        ...
        self.mock_db_session.add.assert_called_once_with(recorder.capture_session) # Corrected attribute name
    ...
    def test_recorder_initialization_device_not_found(self, mock_get_device_by_name):
        ...
        with self.assertRaisesRegex(ValueError, "No valid MIDI devices were found or specified for recording."):
            # Corrected argument order for MidiRecorder instantiation
            MidiRecorder(self.project_id, self.selected_device_names, self.session_name, self.mock_db_session)
    ...
    def test_recorder_start_recording_successful(self, mock_datetime_module, mock_open_input):
        ...
        with patch('src.core.midi_capture.get_midi_device_by_name', return_value=self.mock_found_device) as mock_get_device_init:
        ...
        mock_open_input.assert_called_once_with(self.mock_found_device.name, callback=unittest.mock.ANY)
    ...
    def test_recorder_start_recording_port_open_failure(self, mock_open_input_error):
        # Init recorder
        with patch('src.core.midi_capture.get_midi_device_by_name', return_value=self.mock_found_device):
            recorder = MidiRecorder(
                project_id=self.project_id,
                selected_device_names=self.selected_device_names,
                session_name=self.session_name,
    ...
    def test_recorder_midi_callback(self):
        ...
        with patch('src.core.midi_capture.sanitize_filename', side_effect=lambda x: x):
            ...
            self.assertEqual(recorder.midi_files[key_ch0_dev1].tracks[0].name, "TestDevice1_ch0")
            ...
            self.assertEqual(recorder.midi_files[key_ch0_dev1].tracks[0], [msg1_ch0_dev1, msg2_ch0_dev1])
            ...
            self.assertEqual(recorder.midi_files[key_sys_dev1].tracks[0].name, "TestDevice1_chsys")
    ...
    @patch('src.core.midi_capture.datetime', wraps=datetime)
    def test_recorder_stop_recording_successful_with_data(self, mock_datetime_module, mock_bytes_io):
        # Init recorder
        with patch('src.core.midi_capture.get_midi_device_by_name', return_value=self.mock_found_device):
            recorder = MidiRecorder(
                project_id=self.project_id,
        ...
        recorder.target_devices = [self.mock_found_device] # Ensure target_devices is populated
    ...
    def test_recorder_stop_recording_empty_session(self):
        ...
        recorder.active = True
        recorder.midi_inputs = {self.mock_found_device.name: MagicMock(spec=mido.ports.BaseInput)}
        recorder.target_devices = [self.mock_found_device] # Important for the loop in stop_recording
        recorder.midi_files = {}
        ...
        midi_file_add_calls = [c for c in self.mock_db_session.add.call_args_list if hasattr(c[0][0], 'midi_data')]
