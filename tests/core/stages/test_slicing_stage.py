...
def setup_test_recording(db_session: Session) -> RecordingModel:
    ...
    logger.info(f"Created test recording ID: {recording.id} for project ID: {project.id} using file {recording.file_path}")
    return recording

# --- Generated Audio Fixture Paths ---
GENERATED_FIXTURES_DIR = "tests/fixtures/audio/generated"
A4_FILE = os.path.join(GENERATED_FIXTURES_DIR, "A4_440Hz_1sec.wav")
C4_G4_FILE = os.path.join(GENERATED_FIXTURES_DIR, "C4_G4_sequence.wav")
C5_STACCATO_FILE = os.path.join(GENERATED_FIXTURES_DIR, "Short_C5_Staccato.wav")


@pytest.fixture
def setup_real_audio_recording(db_session: Session, audio_file_path: str, recording_name: str) -> RecordingModel:
    """Sets up a Project and a Recording using a real audio file path."""
    if not os.path.exists(audio_file_path):
        pytest.fail(f"Required audio fixture file not found: {audio_file_path}")

    project = ProjectModel(name=f"Test Project for {recording_name}")
    db_session.add(project)
    db_session.commit()

    try:
        sr, frames, channels = _get_audio_details_for_slicing(audio_file_path)
        duration = frames / float(sr) if sr > 0 else 0
    except ValueError as e:
        pytest.fail(f"Could not get audio details for fixture file {audio_file_path}: {e}")

    recording = RecordingModel(
        project_id=project.id,
        name=recording_name,
        file_path=audio_file_path, # Actual path to generated file
        samplerate=sr,
        duration_seconds=duration,
        channels=channels,
        status="pending"
    ...
    db_session.commit()
    db_session.refresh(recording)
    logger.info(f"Created real audio recording ID: {recording.id} ({recording_name}) for project ID: {project.id}")
    return recording


# --- SlicingStage Tests ---

# Test with Mocks (existing tests)
...
def test_slicing_stage_process_success_with_mocks(mock_aubio_src, mock_aubio_notes_obj_class, db_session, setup_test_recording, temp_output_dir_for_samples):
    # This test uses the original setup_test_recording which uses DUMMY_AUDIO_PATH (silent wav)
    # but aubio calls are mocked, so file content doesn't affect note detection here.
    recording = setup_test_recording 
    stage = SlicingStage()

    # --- Mock Aubio (as before) ---
    mock_src_instance = MagicMock()
    mock_src_instance.samplerate = recording.samplerate if recording.samplerate else 44100
    mock_src_instance.channels = recording.channels if recording.channels else 1
    mock_src_instance.duration = int((recording.duration_seconds if recording.duration_seconds else 1.0) * mock_src_instance.samplerate)
    
    num_reads_simulated = 10
    frames_per_read_simulated = stage.default_params["hop_size"]
    simulated_reads = [(MagicMock(), frames_per_read_simulated)] * num_reads_simulated + [(MagicMock(), 0)]
    mock_src_instance.side_effect = simulated_reads
    mock_aubio_src.return_value = mock_src_instance

    mock_notes_instance = MagicMock()
    mock_notes_instance.get_last_pos.side_effect = [50, 150] 
    notes_output_simulation = [[]] * (num_reads_simulated + 1)
    notes_output_simulation[1] = [(60, 100, 50)] 
    notes_output_simulation[4] = [(62, 110, 150)]
