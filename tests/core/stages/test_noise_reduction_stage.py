...
def test_noise_reduction_stage_process_success():
    ...
    output_data_default = stage.process(np.copy(dummy_input_data), params_default, context_default)
    ...
    np.testing.assert_array_almost_equal(output_data_default, expected_output_default, decimal=5)
    ...
    output_data_override = stage.process(np.copy(dummy_input_data), params_override, context_override)
    
    assert isinstance(output_data_override, np.ndarray)
    assert output_data_override.shape == dummy_input_data.shape
    # Placeholder logic is the same, parameters are logged but don't change behavior in placeholder
    expected_output_override = dummy_input_data * 0.98
    np.testing.assert_array_almost_equal(output_data_override, expected_output_override, decimal=5)
...
# Ensure numpy is available for these tests
# (It's added as a project dependency, so this should be fine)
```
