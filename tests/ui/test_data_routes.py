...
def test_view_recording_shows_samples(client: TestClient, session: Session):
    ...
    assert response.status_code == 200
    response_text = response.text
    assert "/path/to/sample1.wav" in response_text
    assert "/path/to/sample2.wav" in response_text
...
def test_view_sample_loads_with_sample(client: TestClient, session: Session):
    ...
    assert response.status_code == 200
    response_text = response.text
    assert f"Sample - {sample.name}" in response_text
    assert "/path/to/sample.wav" in response_text
