...
from src.database.models import Project as ProjectModel, Recording as RecordingModel, Sample as SampleModel
...
def create_project():
    ...
    try:
        ...
        return jsonify({"error": "Could not create project due to an internal error"}), 500
    finally:
        db.close()
...
def add_project_recording(project_id: int):
    ...
    if not os.path.exists(project_upload_dir):
        try:
            ...
            current_app.logger.error(f"Error creating upload directory {project_upload_dir}: {e}", exc_info=True)
    ...
    try:
        ...
        current_app.logger.error(f"Error saving uploaded file to {file_path}: {e}", exc_info=True)
    ...
    try:
        ...
        current_app.logger.info(f"Recording '{new_recording_model.name}' (ID: {new_recording_model.id}) added to project {project_id}.")
        return jsonify(model_to_dict(new_recording_model)), 201
    except FileNotFoundError as e: # Should be caught by add_recording, but good as a safeguard
        current_app.logger.error(f"File not found during add_recording call: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 400 # Or 500 if it implies internal state issue
    except Exception as e:
        current_app.logger.error(f"Error adding recording DB entry for project {project_id} and file {file_path}: {e}", exc_info=True)
        ...
        if os.path.exists(file_path):
            try:
                ...
                current_app.logger.error(f"Error cleaning up orphaned file {file_path}: {rm_e}", exc_info=True)
...
def process_recording_endpoint(recording_id: int):
    ...
    try:
        ...
        from src.core.stage_runner import execute_stage_chain, STAGE_REGISTRY # Import STAGE_REGISTRY for error messages
        ...
        return jsonify({
            ...
            "result": processing_result if isinstance(processing_result, (list, dict)) else str(processing_result) # Avoid sending complex objects directly
        }), 200
