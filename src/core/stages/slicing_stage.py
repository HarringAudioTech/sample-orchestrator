...
def _get_audio_details_for_slicing(path: str) -> tuple[int, int, int]:
    ...
    try:
        ...
        channels = s.channels
        if channels == 0 : # Aubio might return 0 channels for some files it can't fully parse
            logger.warning(f"Aubio reported 0 channels for {path}. Attempting fallback with wave module.")
            raise RuntimeError("Aubio reported 0 channels.") # Force fallback
        logger.info(f"Audio details from Aubio for {path}: SR={samplerate}, Frames={total_frames}, Channels={channels}")
        return samplerate, total_frames, channels
    except Exception as e_aubio:
        logger.warning(f"Error getting full audio details for {path} with aubio ({e_aubio}). Falling back to wave module.")
        try:
            with wave.open(path, 'rb') as wf:
                samplerate = wf.getframerate()
                total_frames = wf.getnframes()
                channels = wf.getnchannels()
                logger.info(f"Audio details from wave module for {path}: SR={samplerate}, Frames={total_frames}, Channels={channels}")
                return samplerate, total_frames, channels
        except Exception as e_wave:
            logger.error(f"Critical error getting audio details for {path} with wave (fallback): {e_wave}", exc_info=True)
            raise ValueError(f"Could not determine audio details (samplerate, frames, channels) for {path} using aubio or wave.") from e_wave
...
class SlicingStage(AudioProcessingStage):
    ...
    def process(self, data: str, params: dict, context: dict = None) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"[{self.name}] Stage starting. Input file: {data}, Params: {params}, Context keys: {list(context.keys() if context else [])}")
        ...
        recording = db_session.query(RecordingModel).filter(RecordingModel.id == recording_id).first()
        if not recording:
        ...
        try:
            ...
            if samplerate == 0 or num_channels == 0: # total_frames_in_source could be 0 for empty file
                 raise RuntimeError(f"Could not determine valid samplerate ({samplerate}Hz) or channels ({num_channels}) for {data}.")

            # --- Aubio Setup ---
            hop_size = params.get("hop_size", self.default_params["hop_size"])
            win_size = params.get("window_size", self.default_params["window_size"]) # May not be used by 'default' notes method
            ...
            notes_obj.set_param("silence", params.get("silence_threshold_db", self.default_params["silence_threshold_db"]))
            min_ioi_calc = (hop_size * params.get("min_ioi_seconds_factor", self.default_params["min_ioi_seconds_factor"])) / float(actual_samplerate)
            ...
            while True:
                ...
                for note_event in new_note_events:
                    onset_frame = frames_read_count - read + int(notes_obj.get_last_pos())
            ...
            logger.info(f"[{self.name}] Detected {len(detected_notes_list)} potential notes in recording {recording_id}.")
            ...
            for i in range(len(detected_notes_list)):
                ...
                if i + 1 < len(detected_notes_list):
                    ...
                    detected_notes_list[i]["end_frame"] = max(current_note_start, next_note_start - 1)
                else:
                    estimated_end = current_note_start + actual_samplerate # Approx 1s duration
                    detected_notes_list[i]["end_frame"] = min(estimated_end, frames_read_count)
            ...
            for note_info in detected_notes_list:
                ...
                if end_frame <= start_frame:
                    logger.warning(f"[{self.name}] Skipping note MIDI {midi_pitch} due to invalid frame range (start: {start_frame}, end: {end_frame}).")
                    continue
                ...
                if slice_duration_frames <= 0:
                    logger.warning(f"[{self.name}] Skipping sample for MIDI {midi_pitch} due to zero/negative duration after boundary checks.")
                    continue
                ...
                try:
                    with wave.open(data, 'rb') as wf_in: # 'data' is the input file path
                        ...
                        with wave.open(output_sample_path, 'wb') as wf_out:
                            ...
                            wf_out.setframerate(actual_samplerate) # Use samplerate from aubio processing
                            wf_out.writeframes(slice_audio_data)
                        logger.info(f"[{self.name}] Saved sample: {output_sample_path}")
                except Exception as e_slice:
                    logger.error(f"[{self.name}] Error slicing/saving sample for MIDI {midi_pitch} (frames {start_frame}-{end_frame}): {e_slice}", exc_info=True)
                    continue

                new_sample_db = SampleModel(
                    recording_id=recording_id,
                    name=sample_filename,
                    file_path=os.path.abspath(output_sample_path), # Store absolute path
            ...
            logger.info(f"[{self.name}] Slicing completed for recording {recording_id}. {len(created_samples_info)} samples created.")

        except FileNotFoundError as fnf_error: # Should be caught before this block by os.path.exists(data)
            logger.error(f"[{self.name}] File not found during processing: {fnf_error}", exc_info=True)
            recording.status = "slicing_failed"
            raise
        except ValueError as val_error: # Catch ValueErrors like missing context keys
            logger.error(f"[{self.name}] ValueError during processing: {val_error}", exc_info=True)
            recording.status = "slicing_failed"
            raise
        except RuntimeError as rt_error: # Catch critical errors like inability to get audio details
             logger.error(f"[{self.name}] Runtime error during processing: {rt_error}", exc_info=True)
             recording.status = "slicing_failed"
             raise
        except Exception as e:
            logger.error(f"[{self.name}] Unexpected error during slicing for recording {recording_id}: {e}", exc_info=True)
            recording.status = "slicing_failed"
            # Re-raise to allow stage_runner to catch it
            raise RuntimeError(f"Slicing failed for recording {recording_id} due to: {e}") from e
        finally:
