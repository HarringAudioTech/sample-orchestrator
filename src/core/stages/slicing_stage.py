    ...
    def process(
        ...
        try:
            ...
            if recording.samplerate != actual_samplerate:
                logger.info(f"[{self.name}] Updating recording {recording_id} samplerate from {recording.samplerate} to {actual_samplerate}.")
            ...
            if final_onset_params.get("units") == "frames":
                 ...
                 hop_length = final_onset_params.get('hop_length', 512) # librosa default hop_length for onset_detect
                 onset_event_indices = librosa.onset.onset_detect(y=y, sr=sr, **final_onset_params)
                 onset_samples = librosa.frames_to_samples(onset_event_indices, hop_length=hop_length)
            ...
            logger.info(f"[{self.name}] Detected {len(onset_samples)} onsets in recording {recording_id}.")
            ...
            min_len_samples = int(params.get("min_sample_length_ms", self.default_params["min_sample_length_ms"]) / 1000 * sr)
            max_len_samples = int(params.get("max_sample_length_ms", self.default_params["max_sample_length_ms"]) / 1000 * sr)
            ...
            for i, start_sample_idx in enumerate(onset_samples):
                ...
                end_sample = min(end_sample, total_samples) # Ensure it doesn't exceed audio length

                slice_duration_samples = end_sample - start_sample

                if slice_duration_samples < min_len_samples:
                    logger.debug(f"[{self.name}] Skipping sample {i+1} (onset at {start_sample}) due to short duration ({slice_duration_samples} < {min_len_samples} samples).")
                    continue
                ...
                try:
                    ...
                    logger.error(f"[{self.name}] Error saving sample slice (samples {start_sample}-{end_sample}): {e_slice}", exc_info=True)
