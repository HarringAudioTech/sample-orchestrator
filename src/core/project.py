...
from aubio import source # aubio.notes is not directly used here, but in audio_processor
...
class Project:
    ...
    def __init__(self, project_id: int):
        ...
        try:
            self.project_model = db.query(ProjectModel).filter(ProjectModel.id == project_id).first()
    ...
    def add_recording(self, file_path: str, name: str) -> RecordingModel:
        ...
        try:
            ...
            try:
                ...
                s = source(file_path, 0, 512) # hop_size = 512, samplerate = 0 (use original)
                ...
                with wave.open(file_path, 'rb') as wf:
                    ...
                    if samplerate == 0: # If aubio couldn't determine samplerate (e.g. non-wav)
                        ...
                        print(f"Warning: aubio samplerate {samplerate} and wave module samplerate {rate_wave} differ for {file_path}. Using aubio's.")
            except Exception as e:
                # Log error but proceed to add recording entry without full metadata if necessary
                print(f"Error getting audio properties for {file_path}: {e}. Recording will be added with available metadata.")
    ...
    def list_recordings(self) -> list[RecordingModel]:
        ...
        db: Session = next(db_gen)
        try:
            recordings = db.query(RecordingModel).filter(RecordingModel.project_id == self.project_id).all()
            return recordings
        finally:
            next(db_gen, None)
    ...
    def process_recording(self, recording_id: int, output_sample_dir: str):
        ...
        if not os.path.exists(output_sample_dir):
            try:
                ...
                print(f"Error creating output directory {output_sample_dir}: {e}. Processing aborted.")
        ...
        try:
            recording = db_processing_session.query(RecordingModel).filter(
                RecordingModel.id == recording_id,
                RecordingModel.project_id == self.project_id
            ).first()

            if not recording:
                print(f"Recording with id {recording_id} not found for project {self.project_id}. Processing aborted.")
                return

            # Call the processing function, passing the session for it to use
            detect_and_slice_recording(db_processing_session, recording_id, output_sample_dir)
            ...
            print(f"An unexpected error occurred during process_recording setup for recording {recording_id}: {e}")
