...
from src.database.utils import init_db, get_session_local # Renamed SessionLocal to get_session_local
...
def create_app() -> Flask:
    ...
    with app.app_context():
        ...
        init_db()
        app.logger.info(f"Database initialized. DB located at: {get_session_local().bind.engine.url}")
    ...
    def internal_server_error(error):
        ...
        app.logger.error(f"500 Internal Server Error: {request.path} (Error: {error})", exc_info=True)
        return jsonify({"error": "Internal Server Error", "message": "An unexpected error occurred on the server."}), 500

    @app.route('/')
    def index():
        """A simple root route to indicate the API is running."""
        return jsonify({"message": "Welcome to the Audio Processing and Sample Management API!"})
    ...
    for folder_key in ['UPLOAD_FOLDER', 'SAMPLES_BASE_DIR']:
        ...
        if folder_path and not os.path.exists(folder_path):
            try:
                ...
                current_app_instance.logger.info(f"Successfully created data directory: {folder_path}")
            except OSError as e:
                current_app_instance.logger.error(f"Error creating data directory {folder_path}: {e}", exc_info=True)
