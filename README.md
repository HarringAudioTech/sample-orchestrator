...
*   **Test Audio Fixtures:**
    The project includes a script to generate specific audio files for testing: `tests/fixtures/audio/generate_audio.py`.
    This script creates various WAV files with known audio content (e.g., pure tones at specific frequencies, sequences of notes with defined timings) and saves them into `tests/fixtures/audio/generated/`.
    These generated audio files are crucial for testing the accuracy and behavior of audio processing stages, particularly for validating features like pitch detection and slice timing in the `SlicingStage`.
    To generate these files, you can run the script directly (after installing dependencies):
    ```bash
    poetry run python tests/fixtures/audio/generate_audio.py
    ```
    The generated files are then used by unit tests in `tests/core/stages/test_slicing_stage.py` to verify that the slicing logic performs as expected with predictable audio inputs.
